...
ArrayGL is a high-level graphics library built in C++ and OpenGL that focuses on making simple games or graphical programs easy to make and with
a few lines of code.
...
For contribution ideas check out ```new_features.md``` which has a list of features that has not been implemented yet.
...
int main() {
    ...
    while (run()) {
        // code that needs to be run every frame...
    }
    return 0;
}
```
